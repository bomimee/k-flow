...
export default function ResultResponse({ result }: ResultResponseProps) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'expressions' | 'grammar' | 'vocabulary' | 'practice'>('overview');
    ...
    <div className="space-y-8 text-black">
      ...
      <div className="rounded-lg p-6 bg-[var(--lemon)] shadow-lg">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <h2 className="text-2xl font-bold mb-3">{analysis.video_context.topic}</h2>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            ...
            <ul className="space-y-1">
              {analysis.video_context.key_cultural_notes.map((note, idx) => (
                <li key={idx} className="text-sm text-gray-700 pl-4 border-l-2 border-yellow-400">
                  {note}
                </li>
              ))}
            </ul>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg p-4 shadow">
          <p className="text-2xl font-bold text-blue-600">{analysis.review_summary.total_expressions}</p>
          <p className="text-sm text-gray-600">Expressions</p>
        </div>
        <div className="bg-white rounded-lg p-4 shadow">
          <p className="text-2xl font-bold text-purple-600">{analysis.review_summary.total_grammar_points}</p>
          <p className="text-sm text-gray-600">Grammar Points</p>
        </div>
        <div className="bg-white rounded-lg p-4 shadow">
          <p className="text-sm font-bold text-orange-600">{analysis.review_summary.difficulty_rating}</p>
          <p className="text-sm text-gray-600">Difficulty</p>
        </div>
        <div className="bg-white rounded-lg p-4 shadow">
          <p className="text-2xl font-bold text-green-600">{analysis.review_summary.estimated_study_time}</p>
      ...
      {activeTab === 'overview' && (
        <section className="space-y-6">
          ...
          {analysis.natural_speech_patterns.length > 0 && (
            <div className="bg-white rounded-lg p-6 shadow">
              <h3 className="text-xl font-bold mb-4">💬 Natural Speech Patterns</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {analysis.natural_speech_patterns.slice(0, 4).map((pattern, idx) => (
                  <div key={idx} className="p-3 bg-gray-50 rounded">
                    <p className="font-semibold text-blue-600">{pattern.pattern}</p>
                    <p className="text-sm text-gray-600 mt-1">{pattern.meaning_en}</p>
      ...
      {activeTab === 'expressions' && (
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {analysis.key_expressions.map((item, idx) => (
              ...
              >
                ...
                <p className="text-lg text-gray-700 mb-2">→ {item.meaning_en}</p>
                
                {item.pronunciation_notes && (
                  <div className="bg-yellow-50 p-3 rounded mb-3">
                    <p className="text-sm">🔊 <span className="font-semibold">Pronunciation:</span> {item.pronunciation_notes}</p>
                  </div>
                )}
                
                <div className="bg-blue-50 p-3 rounded mb-3">
                  <p className="text-sm">💡 <span className="font-semibold">Usage:</span> {item.usage_context}</p>
                </div>
                
                <div className="bg-gray-50 p-3 rounded mb-3">
                  <p className="text-sm">📝 <span className="font-semibold">Example:</span> {item.example_in_context}</p>
                </div>
          ...
          {analysis.idiomatic_expressions.length > 0 && (
            <div className="mt-8">
              <h4 className="text-xl font-bold mb-4">🎭 Idiomatic Expressions</h4>
              <div className="space-y-4">
                {analysis.idiomatic_expressions.map((idiom, idx) => (
                  <div key={idx} className="bg-white rounded-lg p-5 shadow">
                    <p className="text-xl font-bold text-purple-600 mb-2">{idiom.idiom}</p>
                    <p className="text-sm text-gray-500 mb-2">Literal: {idiom.literal_translation}</p>
                    <p className="text-gray-700 mb-2">→ {idiom.actual_meaning}</p>
                    {idiom.origin && (
                      <p className="text-sm text-gray-600 bg-purple-50 p-2 rounded">
                        📚 {idiom.origin}
                      </p>
                    )}
                    <p className="text-sm text-gray-700 mt-2 italic">"{idiom.example}"</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {analysis.grammar_points.map((g, idx) => (
              <div key={idx} className="rounded-lg p-6 bg-white shadow">
                <div className="flex items-start justify-between mb-3">
                  <p className="text-xl font-bold text-purple-700">{g.pattern}</p>
                  <span className={`px-3 py-1 text-xs rounded-full ${
                ...
                <div className="bg-purple-50 p-3 rounded mb-3">
                  <p className="text-sm"><span className="font-semibold">Formation:</span> {g.formation}</p>
                </div>
                
                <div className="bg-blue-50 p-3 rounded mb-3">
                  <p className="text-sm"><span className="font-semibold">Function:</span> {g.function}</p>
                </div>
                ...
                <div className="space-y-2 mb-3">
                  ...
                  {g.example_sentences.map((ex, i) => (
                    <div key={i} className="bg-gray-50 p-3 rounded">
                      ...
                      <p className="text-sm text-gray-600">[{ex.romanization}]</p>
                      <p className="text-sm text-gray-700 mt-1">→ {ex.english}</p>
                      <p className="text-xs text-gray-500 mt-1">💡 {ex.breakdown}</p>
                    </div>
                  ))}
                </div>
                ...
                {g.common_mistakes && (
                  <div className="bg-red-50 p-3 rounded border-l-4 border-red-400">
                    <p className="text-sm"><span className="font-semibold">⚠️ Common Mistakes:</span> {g.common_mistakes}</p>
                  </div>
                )}
              </div>
            ))}
          ...
          {analysis.honorifics_analysis && (
            <div className="mt-8 bg-white rounded-lg p-6 shadow">
              ...
              <p className="text-gray-700 mb-4">{analysis.honorifics_analysis.relationship_dynamics}</p>
              <div className="space-y-2">
                {analysis.honorifics_analysis.key_honorific_forms.map((form, idx) => (
                  <div key={idx} className="flex items-center gap-4 p-3 bg-gray-50 rounded">
                    <span className="font-semibold text-blue-600">{form.expression}</span>
                    <span className="text-gray-400">↔</span>
                    <span className="text-gray-600">{form.casual_equivalent}</span>
                    <span className="text-sm text-gray-500 ml-auto">({form.when_to_use})</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                {analysis.pronunciation_guide.map((pron, idx) => (
                  <div key={idx} className="p-3 bg-yellow-50 rounded">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-gray-400">→</span>
                      <span className="text-blue-600 font-semibold">{pron.pronounced}</span>
                    </div>
      ...
      {activeTab === 'vocabulary' && (
        <section className="space-y-6">
          ...
          {Object.entries(analysis.vocabulary_by_category).map(([category, items]) => (
            items && items.length > 0 && (
              <div key={category} className="bg-white rounded-lg p-6 shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {items.map((item, idx) => (
                    <div key={idx} className="p-3 bg-gray-50 rounded hover:bg-gray-100 transition">
                      <p className="font-semibold text-lg">{item.word}</p>
                      <p className="text-gray-700">{item.meaning}</p>
                      {item.conjugation_tip && (
                        <p className="text-sm text-blue-600 mt-1">💡 {item.conjugation_tip}</p>
                      )}
                      {item.usage_note && (
                        <p className="text-sm text-gray-500 mt-1">📝 {item.usage_note}</p>
                      )}
                    </div>
                  ))}
                </div>
        ...
        <section className="space-y-6">
          ...
          {analysis.practice_exercises.fill_in_the_blank.length > 0 && (
            <div className="bg-white rounded-lg p-6 shadow">
              ...
              <div className="space-y-4">
                {analysis.practice_exercises.fill_in_the_blank.map((ex, idx) => (
                  <div key={idx} className="p-4 bg-blue-50 rounded">
                    <p className="font-semibold mb-2">{idx + 1}. {ex.sentence}</p>
                    <details className="mt-2">
                      <summary className="cursor-pointer text-sm text-blue-600 hover:text-blue-800">
                        Show hint
                      </summary>
                      <p className="text-sm text-gray-600 mt-1">💡 {ex.hint}</p>
                    </details>
                    <details className="mt-2">
                      <summary className="cursor-pointer text-sm text-green-600 hover:text-green-800">
                        Show answer
                      </summary>
                      <p className="text-sm font-semibold text-green-700 mt-1">✓ {ex.answer}</p>
                    </details>
                  </div>
                ))}
              </div>
                  ...
                  <div key={idx} className="p-4 bg-purple-50 rounded">
                    ...
                    <details className="mt-3">
                      ...
                      <div className="mt-2 p-2 bg-green-50 rounded">
                        <p className="text-sm font-semibold text-green-700">✓ {ex.correct_answer}</p>
          ...
          {analysis.practice_exercises.translation_practice.length > 0 && (
            <div className="bg-white rounded-lg p-6 shadow">
              <h4 className="text-lg font-bold mb-4">🔄 Translation Practice</h4>
              <div className="space-y-4">
                {analysis.practice_exercises.translation_practice.map((ex, idx) => (
                  <div key={idx} className="p-4 bg-yellow-50 rounded">
                    <p className="font-semibold mb-2">{idx + 1}. {ex.english}</p>
                    <details className="mt-2">
                      <summary className="cursor-pointer text-sm text-blue-600 hover:text-blue-800">
                        Show translation
                      </summary>
                      <div className="mt-2 space-y-1">
                        <p className="text-sm font-semibold text-blue-700">✓ {ex.korean_answer}</p>
                        {ex.alternative_answers.length > 0 && (
                          <div className="text-sm text-gray-600">
                            <p>Also correct:</p>
                            {ex.alternative_answers.map((alt, i) => (
                              <p key={i} className="ml-2">• {alt}</p>
                            ))}
                          </div>
                        )}
                      </div>
          ...
          {analysis.common_mistakes.length > 0 && (
            <div className="bg-white rounded-lg p-6 shadow">
              <h4 className="text-lg font-bold mb-4">⚠️ Common Mistakes to Avoid</h4>
              <div className="space-y-3">
                {analysis.common_mistakes.map((mistake, idx) => (
                  <div key={idx} className="p-4 bg-red-50 rounded border-l-4 border-red-400">
                    <span className="inline-block px-2 py-1 text-xs bg-red-200 text-red-800 rounded mb-2">
                      {mistake.mistake_type}
                    </span>
                    <div className="space-y-2">
                      <p className="text-sm">
                        <span className="text-red-600 font-semibold">❌ Wrong:</span> {mistake.wrong}
                      </p>
                      <p className="text-sm">
                        <span className="text-green-600 font-semibold">✅ Correct:</span> {mistake.correct}
                      </p>
